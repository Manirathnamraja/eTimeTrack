CREATE VIEW vw_MPRL_Employees AS
...
WHERE        (dbo.EmployeeProjects.ProjectId = 1658)
GO

  CREATE VIEW vw_MPRL_EmployeeTimesheetItems AS
...
SELECT DISTINCT ET.[TimesheetID]
  ...
  CREATE VIEW vw_MPRL_ProjectGroups AS 
  SELECT [GroupID]
      ,[ProjectID]
      ,[PartID]
      ,[GroupNo]
	...
	WHERE (ProjectID = 1658)
	GO
  
  CREATE VIEW vw_MPRL_ProjectParts AS 
  SELECT [PartID]
      ,[ProjectID]
      ,[PartNo]
      ,[Name]
  ...
  CREATE VIEW vw_MPRL_Projects AS 
  SELECT [ProjectID]
      ,[ProjectNo]
      ,[OfficeID]
      ,[SeqNo]
  ...
  CREATE VIEW vw_MPRL_ProjectTasks AS 
  SELECT [TaskID]
      ,[ProjectID]
      ,[GroupID]
      ,[TaskNo]
  ...
  CREATE VIEW vw_MPRL_ProjectVariationItems AS 
  ...
  CREATE VIEW vw_MPRL_ProjectVariations AS 
  SELECT [VariationID]
      ,[ProjectID]
      ,[VariationNo]
      ,[RevNo]
	...
	CREATE VIEW vw_MPRL_ProjectTimeCodeConfigs AS 
	SELECT
	...
	CREATE VIEW vw_MPRL_Custom_Report AS 
	SELECT
	...
	WHERE dbo.Projects.ProjectID = 1658
