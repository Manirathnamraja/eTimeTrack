CREATE VIEW vw_APAM_Employees AS
...
WHERE        (dbo.EmployeeProjects.ProjectId = 1509)
GO

  CREATE VIEW vw_APAM_EmployeeTimesheetItems AS
  ...
  WHERE PV.ProjectID = 1509
GO
  
  CREATE VIEW vw_APAM_EmployeeTimesheets AS
SELECT DISTINCT ET.[TimesheetID]
  ...
  CREATE VIEW vw_APAM_ProjectGroups AS 
  SELECT [GroupID]
      ,[ProjectID]
      ,[PartID]
      ,[GroupNo]
	...
	WHERE (ProjectID = 1509)
	GO
  
  CREATE VIEW vw_APAM_ProjectParts AS 
  SELECT [PartID]
      ,[ProjectID]
      ,[PartNo]
      ,[Name]
  ...
  CREATE VIEW vw_APAM_Projects AS 
  SELECT [ProjectID]
      ,[ProjectNo]
      ,[OfficeID]
      ,[SeqNo]
  ...
  CREATE VIEW vw_APAM_ProjectTasks AS 
  SELECT [TaskID]
      ,[ProjectID]
      ,[GroupID]
      ,[TaskNo]
  ...
  CREATE VIEW vw_APAM_ProjectVariationItems AS 
  ...
  CREATE VIEW vw_APAM_ProjectVariations AS 
  SELECT [VariationID]
      ,[ProjectID]
      ,[VariationNo]
      ,[RevNo]
