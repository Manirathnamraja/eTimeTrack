CREATE VIEW vw_IRVE_Employees AS
...
WHERE        (dbo.EmployeeProjects.ProjectId = 1645)
GO

  CREATE VIEW vw_IRVE_EmployeeTimesheetItems AS
...
SELECT DISTINCT ET.[TimesheetID]
  ...
  CREATE VIEW vw_IRVE_ProjectGroups AS 
  SELECT [GroupID]
      ,[ProjectID]
      ,[PartID]
      ,[GroupNo]
	...
	WHERE (ProjectID = 1645)
	GO
  
  CREATE VIEW vw_IRVE_ProjectParts AS 
  SELECT [PartID]
      ,[ProjectID]
      ,[PartNo]
      ,[Name]
  ...
  CREATE VIEW vw_IRVE_Projects AS 
  SELECT [ProjectID]
      ,[ProjectNo]
      ,[OfficeID]
      ,[SeqNo]
  ...
  CREATE VIEW vw_IRVE_ProjectTasks AS 
  SELECT [TaskID]
      ,[ProjectID]
      ,[GroupID]
      ,[TaskNo]
  ...
  CREATE VIEW vw_IRVE_ProjectVariationItems AS 
  ...
  CREATE VIEW vw_IRVE_ProjectVariations AS 
  SELECT [VariationID]
      ,[ProjectID]
      ,[VariationNo]
      ,[RevNo]
	...
	CREATE VIEW vw_IRVE_ProjectTimeCodeConfigs AS 
	SELECT
	...
	CREATE VIEW vw_IRVE_Custom_Report AS 
	SELECT
	...
	WHERE dbo.Projects.ProjectID = 1645
	AND ([Day1Hrs] <> 0 OR [Day2Hrs] <> 0 OR [Day3Hrs] <> 0 OR [Day4Hrs] <> 0 OR [Day5Hrs] <> 0 OR [Day6Hrs] <> 0 OR [Day7Hrs] <> 0)
	GO
	
	CREATE VIEW vw_IRVE_TasksSummary AS 
	SELECT dbo.ProjectParts.PartNo, dbo.ProjectParts.Name AS PartName, dbo.ProjectGroups.GroupNo, dbo.ProjectGroups.Name AS GroupName, dbo.ProjectTasks.TaskNo, dbo.ProjectTasks.Name AS TaskName, dbo.ProjectTasks.Notes, dbo.ProjectTasks.IsClosed
	FROM dbo.ProjectGroups INNER JOIN
		 dbo.ProjectParts ON dbo.ProjectGroups.PartID = dbo.ProjectParts.PartID INNER JOIN
		 dbo.Projects ON dbo.ProjectGroups.ProjectID = dbo.Projects.ProjectID AND dbo.ProjectParts.ProjectID = dbo.Projects.ProjectID INNER JOIN
		 dbo.ProjectTasks ON dbo.ProjectGroups.GroupID = dbo.ProjectTasks.GroupID AND dbo.Projects.ProjectID = dbo.ProjectTasks.ProjectID
	WHERE (dbo.Projects.ProjectID = 1645)
	GO