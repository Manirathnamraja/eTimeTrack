CREATE VIEW vw_RIA_Employees AS
...
WHERE        (dbo.EmployeeProjects.ProjectId = 1433)
GO

  CREATE VIEW vw_RIA_EmployeeTimesheetItems AS
  ...
  WHERE PV.ProjectID = 1433
GO
  
  CREATE VIEW vw_RIA_EmployeeTimesheets AS
SELECT DISTINCT ET.[TimesheetID]
  ...
  CREATE VIEW vw_RIA_ProjectGroups AS 
  SELECT [GroupID]
      ,[ProjectID]
      ,[PartID]
      ,[GroupNo]
	...
	WHERE (ProjectID = 1433)
	GO
  
  CREATE VIEW vw_RIA_ProjectParts AS 
  SELECT [PartID]
      ,[ProjectID]
      ,[PartNo]
      ,[Name]
  ...
  CREATE VIEW vw_RIA_Projects AS 
  SELECT [ProjectID]
      ,[ProjectNo]
      ,[OfficeID]
      ,[SeqNo]
  ...
  CREATE VIEW vw_RIA_ProjectTasks AS 
  SELECT [TaskID]
      ,[ProjectID]
      ,[GroupID]
      ,[TaskNo]
  ...
  CREATE VIEW vw_RIA_ProjectVariationItems AS 
  ...
  CREATE VIEW vw_RIA_ProjectVariations AS 
  SELECT [VariationID]
      ,[ProjectID]
      ,[VariationNo]
      ,[RevNo]
