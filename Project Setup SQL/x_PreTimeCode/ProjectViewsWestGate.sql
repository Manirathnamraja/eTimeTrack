CREATE VIEW vw_WGT_Employees AS
...
WHERE        (dbo.EmployeeProjects.ProjectId = 1398)
GO

  CREATE VIEW vw_WGT_EmployeeTimesheetItems AS
  ...
  WHERE PV.ProjectID = 1398
GO
  
  CREATE VIEW vw_WGT_EmployeeTimesheets AS
SELECT DISTINCT ET.[TimesheetID]
  ...
  CREATE VIEW vw_WGT_ProjectGroups AS 
  SELECT [GroupID]
      ,[ProjectID]
      ,[PartID]
      ,[GroupNo]
	...
	WHERE (ProjectID = 1398)
	GO
  
  CREATE VIEW vw_WGT_ProjectParts AS 
  SELECT [PartID]
      ,[ProjectID]
      ,[PartNo]
      ,[Name]
  ...
  CREATE VIEW vw_WGT_Projects AS 
  SELECT [ProjectID]
      ,[ProjectNo]
      ,[OfficeID]
      ,[SeqNo]
  ...
  CREATE VIEW vw_WGT_ProjectTasks AS 
  SELECT [TaskID]
      ,[ProjectID]
      ,[GroupID]
      ,[TaskNo]
  ...
  CREATE VIEW vw_WGT_ProjectVariationItems AS 
  ...
  CREATE VIEW vw_WGT_ProjectVariations AS 
  SELECT [VariationID]
      ,[ProjectID]
      ,[VariationNo]
      ,[RevNo]
