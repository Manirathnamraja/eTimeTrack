CREATE VIEW vw_NHPA_Employees AS
...
WHERE        (dbo.EmployeeProjects.ProjectId = 1491)
GO

  CREATE VIEW vw_NHPA_EmployeeTimesheetItems AS
  ...
  WHERE PV.ProjectID = 1491
GO
  
  CREATE VIEW vw_NHPA_EmployeeTimesheets AS
SELECT DISTINCT ET.[TimesheetID]
  ...
  CREATE VIEW vw_NHPA_ProjectGroups AS 
  SELECT [GroupID]
      ,[ProjectID]
      ,[PartID]
      ,[GroupNo]
	...
	WHERE (ProjectID = 1491)
	GO
  
  CREATE VIEW vw_NHPA_ProjectParts AS 
  SELECT [PartID]
      ,[ProjectID]
      ,[PartNo]
      ,[Name]
  ...
  CREATE VIEW vw_NHPA_Projects AS 
  SELECT [ProjectID]
      ,[ProjectNo]
      ,[OfficeID]
      ,[SeqNo]
  ...
  CREATE VIEW vw_NHPA_ProjectTasks AS 
  SELECT [TaskID]
      ,[ProjectID]
      ,[GroupID]
      ,[TaskNo]
  ...
  CREATE VIEW vw_NHPA_ProjectVariationItems AS 
  ...
  CREATE VIEW vw_NHPA_ProjectVariations AS 
  SELECT [VariationID]
      ,[ProjectID]
      ,[VariationNo]
      ,[RevNo]
