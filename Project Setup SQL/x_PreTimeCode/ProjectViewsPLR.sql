CREATE VIEW vw_PLR_Employees AS
...
WHERE        (dbo.EmployeeProjects.ProjectId = 1407)
GO

  CREATE VIEW vw_PLR_EmployeeTimesheetItems AS
  ...
  WHERE PV.ProjectID = 1407
GO
  
  CREATE VIEW vw_PLR_EmployeeTimesheets AS
SELECT DISTINCT ET.[TimesheetID]
  ...
  CREATE VIEW vw_PLR_ProjectGroups AS 
  SELECT [GroupID]
      ,[ProjectID]
      ,[PartID]
      ,[GroupNo]
	...
	WHERE (ProjectID = 1407)
	GO
  
  CREATE VIEW vw_PLR_ProjectParts AS 
  SELECT [PartID]
      ,[ProjectID]
      ,[PartNo]
      ,[Name]
  ...
  CREATE VIEW vw_PLR_Projects AS 
  SELECT [ProjectID]
      ,[ProjectNo]
      ,[OfficeID]
      ,[SeqNo]
  ...
  CREATE VIEW vw_PLR_ProjectTasks AS 
  SELECT [TaskID]
      ,[ProjectID]
      ,[GroupID]
      ,[TaskNo]
  ...
  CREATE VIEW vw_PLR_ProjectVariationItems AS 
  ...
  CREATE VIEW vw_PLR_ProjectVariations AS 
  SELECT [VariationID]
      ,[ProjectID]
      ,[VariationNo]
      ,[RevNo]
