CREATE VIEW vw_SWB_Employees AS
...
WHERE        (dbo.EmployeeProjects.ProjectId = 1496)
GO

  CREATE VIEW vw_SWB_EmployeeTimesheetItems AS
  ...
  WHERE PV.ProjectID = 1496
GO
  
  CREATE VIEW vw_SWB_EmployeeTimesheets AS
SELECT DISTINCT ET.[TimesheetID]
  ...
  CREATE VIEW vw_SWB_ProjectGroups AS 
  SELECT [GroupID]
      ,[ProjectID]
      ,[PartID]
      ,[GroupNo]
	...
	WHERE (ProjectID = 1496)
	GO
  
  CREATE VIEW vw_SWB_ProjectParts AS 
  SELECT [PartID]
      ,[ProjectID]
      ,[PartNo]
      ,[Name]
  ...
  CREATE VIEW vw_SWB_Projects AS 
  SELECT [ProjectID]
      ,[ProjectNo]
      ,[OfficeID]
      ,[SeqNo]
  ...
  CREATE VIEW vw_SWB_ProjectTasks AS 
  SELECT [TaskID]
      ,[ProjectID]
      ,[GroupID]
      ,[TaskNo]
  ...
  CREATE VIEW vw_SWB_ProjectVariationItems AS 
  ...
  CREATE VIEW vw_SWB_ProjectVariations AS 
  SELECT [VariationID]
      ,[ProjectID]
      ,[VariationNo]
      ,[RevNo]
