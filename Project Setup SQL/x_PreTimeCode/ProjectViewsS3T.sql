CREATE VIEW vw_S3T_Employees AS
...
WHERE        (dbo.EmployeeProjects.ProjectId = 1447)
GO

  CREATE VIEW vw_S3T_EmployeeTimesheetItems AS
  ...
  WHERE PV.ProjectID = 1447
GO
  
  CREATE VIEW vw_S3T_EmployeeTimesheets AS
SELECT DISTINCT ET.[TimesheetID]
  ...
  CREATE VIEW vw_S3T_ProjectGroups AS 
  SELECT [GroupID]
      ,[ProjectID]
      ,[PartID]
      ,[GroupNo]
	...
	WHERE (ProjectID = 1447)
	GO
  
  CREATE VIEW vw_S3T_ProjectParts AS 
  SELECT [PartID]
      ,[ProjectID]
      ,[PartNo]
      ,[Name]
  ...
  CREATE VIEW vw_S3T_Projects AS 
  SELECT [ProjectID]
      ,[ProjectNo]
      ,[OfficeID]
      ,[SeqNo]
  ...
  CREATE VIEW vw_S3T_ProjectTasks AS 
  SELECT [TaskID]
      ,[ProjectID]
      ,[GroupID]
      ,[TaskNo]
  ...
  CREATE VIEW vw_S3T_ProjectVariationItems AS 
  ...
  CREATE VIEW vw_S3T_ProjectVariations AS 
  SELECT [VariationID]
      ,[ProjectID]
      ,[VariationNo]
      ,[RevNo]
