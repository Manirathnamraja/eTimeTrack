CREATE VIEW vw_BHRD1_Employees AS
...
WHERE        (dbo.EmployeeProjects.ProjectId = 1672)
GO

  CREATE VIEW vw_BHRD1_EmployeeTimesheetItems AS
...
SELECT DISTINCT ET.[TimesheetID]
  ...
  CREATE VIEW vw_BHRD1_ProjectGroups AS 
  SELECT [GroupID]
      ,[ProjectID]
      ,[PartID]
      ,[GroupNo]
	...
	WHERE (ProjectID = 1672)
	GO
  
  CREATE VIEW vw_BHRD1_ProjectParts AS 
  SELECT [PartID]
      ,[ProjectID]
      ,[PartNo]
      ,[Name]
  ...
  CREATE VIEW vw_BHRD1_Projects AS 
  SELECT [ProjectID]
      ,[ProjectNo]
      ,[OfficeID]
      ,[SeqNo]
  ...
  CREATE VIEW vw_BHRD1_ProjectTasks AS 
  SELECT [TaskID]
      ,[ProjectID]
      ,[GroupID]
      ,[TaskNo]
  ...
  CREATE VIEW vw_BHRD1_ProjectVariationItems AS 
  ...
  CREATE VIEW vw_BHRD1_ProjectVariations AS 
  SELECT [VariationID]
      ,[ProjectID]
      ,[VariationNo]
      ,[RevNo]
	...
	CREATE VIEW vw_BHRD1_ProjectTimeCodeConfigs AS 
	SELECT
	...
	CREATE VIEW vw_BHRD1_Custom_Report AS 
	SELECT
	...
	WHERE ProjectUserTypes.ProjectID = 1672
	GO
	
	CREATE VIEW vw_BHRD1_ProjectEmployeeDetails AS 
	SELECT 
	...
	WHERE 
	EmployeeProjects.ProjectID = 1672
	...
	AND ProjectUserTypes.ProjectID = 1672
	GO
	
	CREATE VIEW vw_BHRD1_ProjectUserTimesheetItems AS 
	SELECT 
	...
	LEFT JOIN ProjectUserTypes ON EmployeeProjects.ProjectUserTypeID = ProjectUserTypes.ProjectUserTypeID
	WHERE 
	ProjectTasks.ProjectID = 1672 
	AND EmployeeProjects.ProjectID = 1672
	AND EmployeeProjects.ProjectUserTypeID IS NOT NULL
