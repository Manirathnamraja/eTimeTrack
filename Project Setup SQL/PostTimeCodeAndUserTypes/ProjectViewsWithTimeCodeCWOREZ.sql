CREATE VIEW vw_CWOREZ_Employees AS
...
WHERE        (dbo.EmployeeProjects.ProjectId = 1746)
GO

  CREATE VIEW vw_CWOREZ_EmployeeTimesheetItems AS
...
SELECT DISTINCT ET.[TimesheetID]
  ...
  CREATE VIEW vw_CWOREZ_ProjectGroups AS 
  SELECT [GroupID]
      ,[ProjectID]
      ,[PartID]
      ,[GroupNo]
	...
	WHERE (ProjectID = 1746)
	GO
  
  CREATE VIEW vw_CWOREZ_ProjectParts AS 
  SELECT [PartID]
      ,[ProjectID]
      ,[PartNo]
      ,[Name]
  ...
  CREATE VIEW vw_CWOREZ_Projects AS 
  SELECT [ProjectID]
      ,[ProjectNo]
      ,[OfficeID]
      ,[SeqNo]
  ...
  CREATE VIEW vw_CWOREZ_ProjectTasks AS 
  SELECT [TaskID]
      ,[ProjectID]
      ,[GroupID]
      ,[TaskNo]
  ...
  CREATE VIEW vw_CWOREZ_ProjectVariationItems AS 
  ...
  CREATE VIEW vw_CWOREZ_ProjectVariations AS 
  SELECT [VariationID]
      ,[ProjectID]
      ,[VariationNo]
      ,[RevNo]
	...
	CREATE VIEW vw_CWOREZ_ProjectTimeCodeConfigs AS 
	SELECT
	...
	CREATE VIEW vw_CWOREZ_Custom_Report AS 
	SELECT
	...
	WHERE ProjectUserTypes.ProjectID = 1746
	GO
	
	CREATE VIEW vw_CWOREZ_ProjectEmployeeDetails AS 
	SELECT 
	...
	WHERE 
	EmployeeProjects.ProjectID = 1746
	...
	AND ProjectUserTypes.ProjectID = 1746
	GO
	
	CREATE VIEW vw_CWOREZ_ProjectUserTimesheetItems AS 
	SELECT 
	...
	LEFT JOIN ProjectUserTypes ON EmployeeProjects.ProjectUserTypeID = ProjectUserTypes.ProjectUserTypeID
	WHERE 
	ProjectTasks.ProjectID = 1746 
	AND EmployeeProjects.ProjectID = 1746
	AND EmployeeProjects.ProjectUserTypeID IS NOT NULL
