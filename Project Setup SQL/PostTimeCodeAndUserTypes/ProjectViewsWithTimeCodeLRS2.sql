CREATE VIEW vw_LRS2_Employees AS
...
WHERE        (dbo.EmployeeProjects.ProjectId = 1668)
GO

  CREATE VIEW vw_LRS2_EmployeeTimesheetItems AS
...
SELECT DISTINCT ET.[TimesheetID]
  ...
  CREATE VIEW vw_LRS2_ProjectGroups AS 
  SELECT [GroupID]
      ,[ProjectID]
      ,[PartID]
      ,[GroupNo]
	...
	WHERE (ProjectID = 1668)
	GO
  
  CREATE VIEW vw_LRS2_ProjectParts AS 
  SELECT [PartID]
      ,[ProjectID]
      ,[PartNo]
      ,[Name]
  ...
  CREATE VIEW vw_LRS2_Projects AS 
  SELECT [ProjectID]
      ,[ProjectNo]
      ,[OfficeID]
      ,[SeqNo]
  ...
  CREATE VIEW vw_LRS2_ProjectTasks AS 
  SELECT [TaskID]
      ,[ProjectID]
      ,[GroupID]
      ,[TaskNo]
  ...
  CREATE VIEW vw_LRS2_ProjectVariationItems AS 
  ...
  CREATE VIEW vw_LRS2_ProjectVariations AS 
  SELECT [VariationID]
      ,[ProjectID]
      ,[VariationNo]
      ,[RevNo]
	...
	CREATE VIEW vw_LRS2_ProjectTimeCodeConfigs AS 
	SELECT
	...
	CREATE VIEW vw_LRS2_Custom_Report AS 
	SELECT
	...
	WHERE ProjectUserTypes.ProjectID = 1668
	GO
	
	CREATE VIEW vw_LRS2_ProjectEmployeeDetails AS 
	SELECT 
	...
	WHERE 
	EmployeeProjects.ProjectID = 1668
	...
	AND ProjectUserTypes.ProjectID = 1668
	GO
	
	CREATE VIEW vw_LRS2_ProjectUserTimesheetItems AS 
	SELECT 
	...
	LEFT JOIN ProjectUserTypes ON EmployeeProjects.ProjectUserTypeID = ProjectUserTypes.ProjectUserTypeID
	WHERE 
	ProjectTasks.ProjectID = 1668 
	AND EmployeeProjects.ProjectID = 1668
	AND EmployeeProjects.ProjectUserTypeID IS NOT NULL
