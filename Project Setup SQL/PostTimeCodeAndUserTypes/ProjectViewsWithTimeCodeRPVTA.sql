CREATE VIEW vw_RPVTA_Employees AS
...
WHERE        (dbo.EmployeeProjects.ProjectId = 1696)
GO

  CREATE VIEW vw_RPVTA_EmployeeTimesheetItems AS
...
SELECT DISTINCT ET.[TimesheetID]
  ...
  CREATE VIEW vw_RPVTA_ProjectGroups AS 
  SELECT [GroupID]
      ,[ProjectID]
      ,[PartID]
      ,[GroupNo]
	...
	WHERE (ProjectID = 1696)
	GO
  
  CREATE VIEW vw_RPVTA_ProjectParts AS 
  SELECT [PartID]
      ,[ProjectID]
      ,[PartNo]
      ,[Name]
  ...
  CREATE VIEW vw_RPVTA_Projects AS 
  SELECT [ProjectID]
      ,[ProjectNo]
      ,[OfficeID]
      ,[SeqNo]
  ...
  CREATE VIEW vw_RPVTA_ProjectTasks AS 
  SELECT [TaskID]
      ,[ProjectID]
      ,[GroupID]
      ,[TaskNo]
  ...
  CREATE VIEW vw_RPVTA_ProjectVariationItems AS 
  ...
  CREATE VIEW vw_RPVTA_ProjectVariations AS 
  SELECT [VariationID]
      ,[ProjectID]
      ,[VariationNo]
      ,[RevNo]
	...
	CREATE VIEW vw_RPVTA_ProjectTimeCodeConfigs AS 
	SELECT
	...
	CREATE VIEW vw_RPVTA_Custom_Report AS 
	SELECT
	...
	WHERE ProjectUserTypes.ProjectID = 1696
	GO
	
	CREATE VIEW vw_RPVTA_ProjectEmployeeDetails AS 
	SELECT 
	...
	WHERE 
	EmployeeProjects.ProjectID = 1696
	...
	AND ProjectUserTypes.ProjectID = 1696
	GO
	
	CREATE VIEW vw_RPVTA_ProjectUserTimesheetItems AS 
	SELECT 
	...
	LEFT JOIN ProjectUserTypes ON EmployeeProjects.ProjectUserTypeID = ProjectUserTypes.ProjectUserTypeID
	WHERE 
	ProjectTasks.ProjectID = 1696 
	AND EmployeeProjects.ProjectID = 1696
	AND EmployeeProjects.ProjectUserTypeID IS NOT NULL
