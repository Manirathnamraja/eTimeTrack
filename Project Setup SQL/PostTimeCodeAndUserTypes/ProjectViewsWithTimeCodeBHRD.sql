CREATE VIEW vw_BHRD_Employees AS
...
WHERE        (dbo.EmployeeProjects.ProjectId = 1663)
GO

  CREATE VIEW vw_BHRD_EmployeeTimesheetItems AS
...
SELECT DISTINCT ET.[TimesheetID]
  ...
  CREATE VIEW vw_BHRD_ProjectGroups AS 
  SELECT [GroupID]
      ,[ProjectID]
      ,[PartID]
      ,[GroupNo]
	...
	WHERE (ProjectID = 1663)
	GO
  
  CREATE VIEW vw_BHRD_ProjectParts AS 
  SELECT [PartID]
      ,[ProjectID]
      ,[PartNo]
      ,[Name]
  ...
  CREATE VIEW vw_BHRD_Projects AS 
  SELECT [ProjectID]
      ,[ProjectNo]
      ,[OfficeID]
      ,[SeqNo]
  ...
  CREATE VIEW vw_BHRD_ProjectTasks AS 
  SELECT [TaskID]
      ,[ProjectID]
      ,[GroupID]
      ,[TaskNo]
  ...
  CREATE VIEW vw_BHRD_ProjectVariationItems AS 
  ...
  CREATE VIEW vw_BHRD_ProjectVariations AS 
  SELECT [VariationID]
      ,[ProjectID]
      ,[VariationNo]
      ,[RevNo]
	...
	CREATE VIEW vw_BHRD_ProjectTimeCodeConfigs AS 
	SELECT
	...
	CREATE VIEW vw_BHRD_Custom_Report AS 
	SELECT
	...
	WHERE ProjectUserTypes.ProjectID = 1663
	GO
	
	CREATE VIEW vw_BHRD_ProjectEmployeeDetails AS 
	SELECT 
	...
	WHERE 
	EmployeeProjects.ProjectID = 1663
	...
	AND ProjectUserTypes.ProjectID = 1663
	GO
	
	CREATE VIEW vw_BHRD_ProjectUserTimesheetItems AS 
	SELECT 
	...
	LEFT JOIN ProjectUserTypes ON EmployeeProjects.ProjectUserTypeID = ProjectUserTypes.ProjectUserTypeID
	WHERE 
	ProjectTasks.ProjectID = 1663 
	AND EmployeeProjects.ProjectID = 1663
	AND EmployeeProjects.ProjectUserTypeID IS NOT NULL
