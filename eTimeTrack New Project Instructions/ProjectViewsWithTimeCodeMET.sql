CREATE VIEW vw_MET_Employees AS
...
WHERE        (dbo.EmployeeProjects.ProjectId = 1541)
GO

  CREATE VIEW vw_MET_EmployeeTimesheetItems AS
...
SELECT DISTINCT ET.[TimesheetID]
  ...
  CREATE VIEW vw_MET_ProjectGroups AS 
  SELECT [GroupID]
      ,[ProjectID]
      ,[PartID]
      ,[GroupNo]
	...
	WHERE (ProjectID = 1541)
	GO
  
  CREATE VIEW vw_MET_ProjectParts AS 
  SELECT [PartID]
      ,[ProjectID]
      ,[PartNo]
      ,[Name]
  ...
  CREATE VIEW vw_MET_Projects AS 
  SELECT [ProjectID]
      ,[ProjectNo]
      ,[OfficeID]
      ,[SeqNo]
  ...
  CREATE VIEW vw_MET_ProjectTasks AS 
  SELECT [TaskID]
      ,[ProjectID]
      ,[GroupID]
      ,[TaskNo]
  ...
  CREATE VIEW vw_MET_ProjectVariationItems AS 
  ...
  CREATE VIEW vw_MET_ProjectVariations AS 
  SELECT [VariationID]
      ,[ProjectID]
      ,[VariationNo]
      ,[RevNo]
	...
	CREATE VIEW vw_MET_ProjectTimeCodeConfigs AS 
	SELECT
	...
	CREATE VIEW vw_MET_Custom_Report AS 
	SELECT
	...
	WHERE ProjectUserTypes.ProjectID = 1541
	GO
	
	CREATE VIEW vw_MET_ProjectEmployeeDetails AS 
	SELECT 
	...
	WHERE 
	EmployeeProjects.ProjectID = 1541
	...
	AND ProjectUserTypes.ProjectID = 1541
	GO
	
	CREATE VIEW vw_MET_ProjectUserTimesheetItems AS 
	SELECT 
	...
	LEFT JOIN ProjectUserTypes ON EmployeeProjects.ProjectUserTypeID = ProjectUserTypes.ProjectUserTypeID
	WHERE 
	ProjectTasks.ProjectID = 1541 
	AND EmployeeProjects.ProjectID = 1541
	AND EmployeeProjects.ProjectUserTypeID IS NOT NULL
